program testmm
        ...
        double precision, dimension(longueur,4*longueur)::inter
        ...
        !$acc parallel private(i,j,indice,indice2,j2,inter,inter2) vector_length(32)
        !$acc cache(inter2(longueur,longueur),inter(longueur,128))
        !$acc loop gang 
        do i = 1, 100000,32*4
          !$acc loop vector
          do j=1,32 
          j2=1
          inter(j,1:128)=v1(j,i:i+128)
          inter2(j,:)=a(j,:)
          indice=i+4*(j-1)
if (indice.le.99997) v2(:,indice:indice+3)=matmul(inter2,inter(:,4*(j-1)+1:4*j))
          !!if (indice .le.99997) then
          !!  v2(:, indice:indice+3) = matmul(inter2,inter(:,4*(j-1)+1:4*j))
          !!elseif (indice .le.99998) then
          !!  v2(:, indice:indice+2) = matmul(inter2,inter(:,4*(j-1)+1:4*j-1))
          !!elseif (indice .le.99999) then
          !!  v2(:, indice:indice+1) = matmul(inter2,inter(:,4*(j-1)+1:4*j-2))
          !!elseif (indice .le. 100000) then
          !!  v2(:, indice) = matmul(inter2,inter(:,4*(j-1)+1))
          !!endif
          enddo
        enddo
        !$acc end parallel
