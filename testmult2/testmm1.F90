program testmm
        ...
        do i = 1, 100000,32*4
          ...
          indice2=i+32*(j2-1)
                indice=indice2+j-1
                if (indice .le.100000) v2(:, indice) = matmul(inter2, inter(:,j+32))

          j2=3
          indice2=i+32*(j2-1)
                indice=indice2+j-1
                if (indice .le.100000) v2(:, indice) = matmul(inter2, inter(:,j+64))
          j2=4
          indice2=i+32*(j2-1)
                indice=indice2+j-1
                if (indice .le.100000) v2(:, indice) = matmul(inter2, inter(:,j+96))
          enddo
        enddo
        !$acc end parallel
