program testmm
        ...
        double precision,dimension (longueur,nbvect) :: v1,v2b, v2
        double precision, parameter ::alpha=1.0
        double precision, parameter ::beta=0.0
        integer :: i,j,nbcolonnes
        ...
        !$acc parallel private(i,j,nbcolonnes,inter,inter2) vector_length(32)
        !$acc cache(inter(longueur,longueur),inter2(longueur,longueur))
        !$acc loop gang
        do i = 1, nbvect,32
          nbcolonnes=min(32,nbvect-i+1)
          !$acc loop vector
          do j=1,32
                inter2(j,1:nbcolonnes)=a(j,1:nbcolonnes)
                inter(j,1:nbcolonnes)=v1(j,i:i+nbcolonnes-1)
                if (j .le. nbcolonnes) v2(:, i+j-1) = matmul(inter2, inter(:,j))
          enddo
        enddo
        !!!!$acc end kernels
        !$acc end parallel
...
program testmm
        ...
        !$acc parallel private(i,j,indice,a) vector_length(64)
        !$acc cache(a(longueur,longueur))
        !$acc loop gang 
        do i = 1, 100000,64
          !$acc cache(v1(longueur,i:i+64))
          !$acc loop vector
          do j = 1,64
                indice=i+j-1
                if (indice .le.100000) v2(:, indice) = matmul(a, v1(:, indice))
          enddo
        enddo
        !$acc end parallel
