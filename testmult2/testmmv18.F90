...
#define dynamique 0
        ...
        double precision, parameter :: zdbt=0.5d0 !!zdbt=6.9123d0
        integer(kind=4)             :: in
        ...
        do jsp=1,indiceSpec
          do jlev=1,indiceLev
             intermediaire=sivp(jlev)*0.231d0
        ...
        do jsp=1,indiceSpec
          do jlev=1,indiceLev
            v6(jsp,jlev)=v5(jsp,jlev)*0.4107d0*0.4107d0 
          enddo
        enddo
        !$acc end kernels
        ...
        !$acc loop gang vector private(inter1,inter2)
          do jsp=1,indiceSpec
            ...
            intermediaire=zdbt*rlapdi(in)
            do jlev=1,indiceLev
              v1b(jsp,jlev)=v3b(jsp,jlev)-intermediaire*v1b(jsp,jlev)!!v6b v1b
            enddo
            ...
            do jlev=1,indiceLev
              ...
              !!v4b(jsp,jlev)=v2b(jsp,jlev)*(intermediaire*rlapdi(nvalue(jsp)))
            enddo
            inter1(1:indiceLev)=v4b(jsp,1:indiceLev)   !!v4b(jsp,1:indiceLev)
            inter2=matmul(bt,inter1)
            v5b(jsp,1:indiceLev)=inter2(1:indiceLev)
            do jlev=1,indiceLev
              v6b(jsp,jlev)=v5b(jsp,jlev)*0.4107d0*0.4107d0 
            enddo
        enddo
        !$acc end parallel
