program testmm
        ...
        double precision, allocatable ::v1(:,:),v2(:,:),v2b(:,:),v3(:,:)
        ...
        allocate(v1(longueur,nbvect),v2(longueur,nbvect),v2b(longueur,nbvect),v3(longueur,nbvect))
        ...
        !$acc data copyin(a,v1,v3) copyout(v2,v2b) create(inter,inter2,inter4)
        ...
        if (.false.) then
        !$acc parallel private(i,j,m,nbcolonnes,m,compteurboucle,comptmult,inter,inter2) vector_length(32)
        ...
        !$acc loop vector private(inter4)
          ...
          do j=1,32 
            ...
            do m=1,nbcolonnes
              ...
              enddo
            enddo          
            v2b(j,i:i+nbcolonnes-1)=inter4(1:nbcolonnes)
          enddo
        endif
        ...
        !$acc loop vector private(inter4)
        do j=1,32
          do comptmult=1,nbcolonnes
            v3(j,i+comptmult-1)=2.0*v2b(j,i+comptmult-1)
          enddo
        enddo
        endif
     ...
     !!   !$acc cache(inter2(longueur,longueur),inter(longueur,longueur))
     !!   !$acc parallel private(i,j,m,nbcolonnes,compteurboucle,comptmult) vector_length(32)
        !$acc loop gang 
        do i = 1, nbvect,32
          nbcolonnes=min(nbvect-i+1,32)
          !!!!!!$acc loop vector !!private(inter4)
        ...
        do j=1,32
          do comptmult=1,nbcolonnes
            v3(j,i+comptmult-1)=2.0*v2(j,i+comptmult-1)
          enddo
        enddo
        endif
